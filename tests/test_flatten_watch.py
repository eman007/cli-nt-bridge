...
def test_flatten_build_request():
    req = flatten.build_flatten_request("id1", "SimAccount2", "MNQ 06-26")
    assert req["kind"] == "flatten"
    assert req["account"] == "SimAccount2"
...
def test_run_flatten_writes_trigger_and_reads_result(monkeypatch, tmp_path):
    ...
    payload = flatten.run_flatten("SimAccount2", "MNQ 06-26", timeout=1.0)
...
def test_scan_once_scopes_to_watched_accounts():
    ...
    findings = watch.scan_once(["SimAccount2"], _state_fn=lambda: state)
    assert len(findings) == 1
    assert findings[0]["account"] == "SimAccount2"
...
def test_protected_position_never_killed():
    findings = [{"account": "SimAccount2", "instrument": "MNQ 06-26",
                 "marketPosition": "Long", "quantity": 1, "protected": True}]
    killed = watch.watch(["SimAccount2"], grace_seconds=0.0, max_iterations=3,
...
def test_naked_within_grace_not_killed_then_killed_after_grace():
    ...
    killed = watch.watch(
        ["SimAccount2"], grace_seconds=20.0, max_iterations=2,
    ...
    assert flat_calls == [("SimAccount2", "MNQ 06-26")]
    assert len(killed) == 1
