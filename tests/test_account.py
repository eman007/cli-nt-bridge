...
def test_build_request_includes_account_filter():
    req = account.build_account_request("id1", "SimAccount2")
    assert req["id"] == "id1"
...
def test_open_position_visible_when_substream_would_have_stalled():
    """The exact gap this command closes: an open position the ZMQ channel missed."""
    res = account.parse_account_response(
        {
            "status": "ok",
            "accounts": [
    ...
    pos = res.account("SimAccount2")["positions"]
...
def test_run_account_state_writes_trigger_and_reads_result(monkeypatch, tmp_path):
    ...
    ntio.atomic_write_json(
        ...
        {"id": "ac1", "status": "ok", "accounts": [{"name": "SimAccount2"}]},
    )
    payload = account.run_account_state("SimAccount2", timeout=1.0)
    assert payload["status"] == "ok"
    assert payload["accounts"][0]["name"] == "SimAccount2"
    ...
    assert req["kind"] == "account" and req["account"] == "SimAccount2"
